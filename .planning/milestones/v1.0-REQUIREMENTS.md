# Requirements Archive: rs485-logger v1.0

**Archived:** 2026-04-03
**Milestone:** v1.0 MVP
**Status:** ✅ COMPLETE — all 16 v1 requirements satisfied
...
- [x] **CFG-02**: Operator can configure serial port path, baud rate, and parity in TOML
  - *Note: parity field is absent from SerialConfig (PZEM-016 8N1 default works implicitly via tokio_serial default). Partially satisfied — deferred to v1.1.*
...
- [x] **STOR-01**: Daemon writes each device's readings to InfluxDB 3 via HTTP POST to `/api/v3/write_lp` using line protocol, with the device name as the measurement name *(checkbox was stale — implemented and verified in Phase 2)*
- [x] **STOR-02**: Daemon uses `Authorization: Bearer <token>` header for InfluxDB writes *(checkbox was stale — implemented and verified in Phase 2)*
- [x] **STOR-03**: All numeric fields are written as `f64` floats (never integers) to prevent InfluxDB 3 field type conflicts *(checkbox was stale — implemented and verified in Phase 2)*
- [x] **STOR-04**: Daemon logs InfluxDB write failures and continues polling (write errors do not crash or stall the daemon) *(checkbox was stale — implemented in Phase 3)*
...
- [x] **DOC-01**: Comprehensive E2E README.md manual (hardware wiring → deployment → InfluxDB verification) — Phase 5
- [x] **SCHED-01**: Daily energy reset via Modbus FC 0x42 at configurable timezone/time — Phase 6
- [x] **REL-01**: Daemon reliability hardening — all 14 daemon reliability findings resolved (HTTP timeouts, serial recovery, config validation, log rotation, git hygiene) — Phase 7
...
| CFG-02 | Phase 1 | ✅ Complete (parity implicit — see note) |
| CFG-03 | Phase 1 | ✅ Complete |
| CFG-04 | Phase 1 | ✅ Complete |
| CFG-05 | Phase 1 | ✅ Complete |
| POLL-01 | Phase 3 | ✅ Complete |
...
**Coverage:**
- v1 requirements: 16 total (+ 3 added during development = 19 shipped)
- All mapped to phases: 19 ✓
- Unmapped: 0 ✓
